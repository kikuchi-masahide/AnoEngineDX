...
void Window::CreateSwapChain(DX12::Master& dx12, std::shared_ptr<DX12::CommandQueue> cmd_queue)
{
	assert(!swapchain_);
...
void Window::SetRenderTarget(std::shared_ptr<DX12::GraphicsCommandList> cmd_list) const
{
	assert(swapchain_);
...
void Window::SetRenderTarget(std::shared_ptr<DX12::GraphicsCommandList> cmd_list,
...
void Window::SetBackbufferStateBarrier(std::shared_ptr<DX12::GraphicsCommandList> cmd_list,
...
void Window::ClearBackbuffer(std::shared_ptr<DX12::GraphicsCommandList> cmd_list, float r, float g, float b) const
{
	assert(swapchain_);
	auto bbindex = swapchain_->GetCurrentBackBufferIndex();
	auto rtv_desc = swapchain_->GetDescriptorHeap();
...
std::weak_ptr<DX12::SwapChain> Window::GetSwapchain() const
{
	return swapchain_;
}
